...
function PrintReportQuestionDetails(qstNum,question)
{
	...
	htmlHeader = "<div class='panel-heading'><b>Domanda " + textToDisplay + "</b></div>";
	textHtml += htmlHeader;
	...
	if (question.valid === 3)
	{
		if (question.minScore === 0) {
			if (question.typeOfQuestion === QMAKE_MATCHING) {
				textHtml += "<p><i>Affermazioni corrette : " + question.nScore + "/" + question.maxScore + "</i></p>";
			}
			else {
				textHtml += "<p><i>Punteggio : " + question.nScore + "/" + question.maxScore + "</i></p>";
			}
		}
		else {
			textHtml += "<p><i>Punteggio : " + question.nScore + " [" + question.minScore + " , " + question.maxScore + "]</i></p>";
		}
	}
	...
	$.ajax({
		...
		complete: function (xhr, textStatus) {
			if (xhr.status === 401) {
				...
				PrintError('Errore di login, http status = ' + xhr.status, 'R');
			}
		}
	});
}
